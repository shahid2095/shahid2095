...
<?php if($id>1)
{
    ...
    echo "<a href='?id=".($id-1)."&status=".$_GET['status']."&limit=".$_GET['limit']."' ><li class='pagination'>PREVIOUS</li></a>";
}
...
//show all the page link with page number. When click on these numbers go to particular page. 
        for($i=1;$i<=$total;$i++)
        {
            ...
            else { echo "<a href='?id=".$i."&status=".$_GET['status']."&limit=".$_GET['limit']." '><li class='pagination'>".$i."</li></a>"; }
        }
if($id!=$total)
{
    ////Go to previous page to show next 10 items.
    echo "<a href='?id=".($id+1)."&status=".$_GET['status']."&limit=".$_GET['limit']."' ><li class='pagination'>NEXT</li></a>";
}
...
<div class="col-lg-2">
	<div class="row">
	   ...
	   <a href="ordersnew.php?status=&limit=<?php
			if(isset($_GET['limit']))
					{
					...
					?>"><div class="status btn btn-md btn-default">All</div></a>

	   <a href="ordersnew.php?status=new&limit=<?php
			if(isset($_GET['limit']))
					{
					...
					?>"><div class="status  btn btn-md btn-success">New Orders</div></a>
			
	   <a href="ordersnew.php?status=pending&limit=<?php
			if(isset($_GET['limit']))
					{
					...
					?>"><div class="status btn btn-md btn-info">Pending</div></a>
	   <a href="ordersnew.php?status=canceled&limit=<?php
			if(isset($_GET['limit']))
					{
					...
					?>"><div class="status btn btn-md btn-danger">Cancelled</div></a>
					
		<a href="ordersnew.php?status=return&limit=<?php
			if(isset($_GET['limit']))
					{
					...
					?>"><div class="status btn btn-md btn-warning">Return</div></a>
					</div>
		   	</div>
		   </div>
		   <div class="col-md-6">
		   		<div class="box-gray">
					   ...
					   <div class="highlight" align="center">78</div>
					   </div>
	   			</div>
		   </div>
	   </div>
	<div class="box-gray">
	   ...
	   <form>
		...
		<input type="hidden" name="filter_type" value="limit">			
	   	<input type="submit" name="submit" class="submit" value="Limit">
			   	</div>
